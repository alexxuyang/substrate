	...
	-> Option<UnappliedSlash<T::AccountId, BalanceOf<T>>>
{
	...
	debug::info!(target: "staking-slashing", "compute_slash-1");
	...
	debug::info!(target: "staking-slashing", "compute_slash-2");
	...
	debug::info!(target: "staking-slashing", "compute_slash-3");
	...
	// apply slash to validator.
	{
		...
		if target_span == Some(spans.span_index()) {
			debug::info!(target: "staking-slashing", "compute_slash-4");
			...
			if T::SessionInterface::disable_validator(stash).unwrap_or(false) {
				debug::info!(target: "staking-slashing", "compute_slash-5");
	...
	debug::info!(target: "staking-slashing", "compute_slash-6");
...
pub fn do_slash<T: Trait>(
	...
	debug::info!(target: "staking-slashing", "do_slash-1");
	...
	debug::info!(target: "staking-slashing", "do_slash-2");
	...
	debug::info!(target: "staking-slashing", "do_slash-3");

	let value = ledger.slash(value, T::Currency::minimum_balance());

	if !value.is_zero() {
		debug::info!(target: "staking-slashing", "do_slash-4");
		...
		debug::info!(target: "staking-slashing", "do_slash-5");
		...
		debug::info!(target: "staking-slashing", "do_slash-6");
	}
}
...
pub(crate) fn apply_slash<T: Trait>(unapplied_slash: UnappliedSlash<T::AccountId, BalanceOf<T>>) {
	...
	debug::info!(target: "staking-slashing", "apply_slash-1");
	...
	debug::info!(target: "staking-slashing", "apply_slash-2");

	for &(ref nominator, nominator_slash) in &unapplied_slash.others {
		debug::info!(target: "staking-slashing", "apply_slash-3");
	...
	debug::info!(target: "staking-slashing", "apply_slash-4");
