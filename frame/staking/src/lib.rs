	...
	Balance: AtLeast32Bit + Saturating + Copy,
{
	...
	fn slash(
		...
		debug::info!(target: "staking", "StakingLedger->slash-1");
		...
		debug::info!(target: "staking", "StakingLedger->slash-2");
...
impl<T: Trait> Module<T> {
	...
	fn start_session(start_session: SessionIndex) {
		debug::info!(target: "staking", "start_session-1");
	...
	fn end_session(session_index: SessionIndex) {
		debug::info!(target: "staking", "end_session-1");
	...
	fn start_era(start_session: SessionIndex) {
		debug::info!(target: "staking", "start_era-1");
		...
		debug::info!(target: "staking", "start_era-2");
	...
	fn end_era(active_era: ActiveEraInfo<MomentOf<T>>, _session_index: SessionIndex) {
		...
		if let Some(active_era_start) = active_era.start {
			debug::info!(target: "staking", "end_era-1");
	...
	fn new_era(start_session_index: SessionIndex) -> Option<Vec<T::AccountId>> {
		debug::info!(target: "staking", "new_era-1");
	...
	fn apply_unapplied_slashes(active_era: EraIndex) {
		debug::info!(target: "staking", "apply_unapplied_slashes-1");

		let slash_defer_duration = T::SlashDeferDuration::get();
		<Self as Store>::EarliestUnappliedSlash::mutate(|earliest| if let Some(ref mut earliest) = earliest {
			debug::info!(target: "staking", "apply_unapplied_slashes-2");

			let keep_from = active_era.saturating_sub(slash_defer_duration);
			for era in (*earliest)..keep_from {
				debug::info!(target: "staking", "apply_unapplied_slashes-3");

				let era_slashes = <Self as Store>::UnappliedSlashes::take(&era);
				for slash in era_slashes {
					debug::info!(target: "staking", "apply_unapplied_slashes-4");
...
impl<T: Trait> SessionManager<T::AccountId, Exposure<T::AccountId, BalanceOf<T>>> for Module<T> {
	...
	{
		debug::info!(target: "staking", "new_session-1");
	...
	T::ValidatorIdOf: Convert<<T as frame_system::Trait>::AccountId, Option<<T as frame_system::Trait>::AccountId>>
{
	fn on_offence(
		...
		debug::info!(target: "staking", "on_offence-1");
		...
		debug::info!(target: "staking", "on_offence-2");
		...
		debug::info!(target: "staking", "on_offence-3");
		...
		debug::info!(target: "staking", "on_offence-4");

		for (details, slash_fraction) in offenders.iter().zip(slash_fraction) {
			debug::info!(target: "staking", "on_offence-5");
			...
			debug::info!(target: "staking", "on_offence-6");

			if let Some(mut unapplied) = unapplied {
				debug::info!(target: "staking", "on_offence-7");

				unapplied.reporters = details.reporters.clone();
				if slash_defer_duration == 0 {
					debug::info!(target: "staking", "on_offence-8");

					// apply right away.
					slashing::apply_slash::<T>(unapplied);
				} else {
					debug::info!(target: "staking", "on_offence-9");
